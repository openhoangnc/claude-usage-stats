...
各プログレスバーには、そのウィンドウの経過時間の位置を示す小さな目盛り —— **ペース
マーカー** が入ります。バーがマーカーより手前なら時間の経過より遅いペースで消費して
おり、マーカーを超えていればリセット前に使い切る見込みです。（まだ使われていない
ウィンドウにはリセット時刻がないため、マーカーもリセット行も表示されません。）
